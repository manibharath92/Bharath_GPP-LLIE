...
def patchify_numpy(img, patchify_factor=32):
    ...
    for i in range(patchify_factor):
        for j in range(patchify_factor):
            a = img_padding[int(patch_h*(i)):int(patch_h*(i+1)), int(patch_w*(j)):int(patch_w*(j+1)), :]
            #print(a.shape)
            output.append(a)
...
def eval_model(args):
    ...
    for image_path, _ in zip(image_paths, range(len(image_paths))):
        ...
        image_tensor_all = result = torch.ones((1, 3, 336, 336)).cuda()


        for i in range(len(patch_images)):
            h, w, c = patch_images[i].shape
            result = torch.ones((3, h, w)).cuda()
            image = Image.fromarray(patch_images[i])
        
            image_tensor = image_processor.preprocess(image, return_tensors='pt')['pixel_values'].half().cuda()
            image_tensor_all = torch.cat([image_tensor_all, image_tensor], dim=0)
            
        image_tensor_all = image_tensor_all[1:, :, :, :].half().cuda()

        for i in range(len(patch_images)):
        
            image_tensor = image_tensor_all[i, :, :, :].unsqueeze(0)
            ...
            probs, inds = output_logits1.sort(dim=-1,descending=True)
            good1, poor1 = output_logits1[0,1781].item(), output_logits1[0,6460].item()
            score1 = my_softmax(good1, poor1)
            score1_t = torch.tensor(score1)
            result[1, :, :] = score1_t
            ...
            probs, inds = output_logits2.sort(dim=-1,descending=True)
            good2, poor2 = output_logits2[0,1781].item(), output_logits2[0,6460].item()
            score2 = my_softmax(good2, poor2)
            score2_t = torch.tensor(score2)
            result[2, :, :] = score2_t
        ...
        map_h = []
        for j in range(patchify_factor):
            initial = output[int(patchify_factor*(j))]
            for k in range(1, patchify_factor):
                initial = torch.cat([initial, output[int(patchify_factor*j+k)].to(initial.device)], dim=-1)
            map_h.append(initial)

        final_map = map_h[0]
        for s in range(1, len(map_h)):
            final_map = torch.cat([final_map, map_h[s].to(final_map.device)], dim=-2)
        
        final_map = torch.unsqueeze(final_map, dim=0)
        final_map = final_map[:, :, padding_params[0]:final_map.shape[2] - padding_params[1], padding_params[2]:final_map.shape[3] - padding_params[3]]

        save_path = os.path.join(out_dir, os.path.basename(image_path).split('.')[0]+'.pt')
        torch.save(final_map, save_path)
